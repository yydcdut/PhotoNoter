...
Java_com_yydcdut_note_utils_RxImageBlur_blur(JNIEnv *env, jobject instance, jobject bitmap,
                                             jint r) {
    ...
    if ((ret = AndroidBitmap_lockPixels(env, bitmap, &pixelsIn)) < 0) {
        ...
        __android_log_write(ANDROID_LOG_ERROR, "yuyidong", "AndroidBitmap_lockPixels failed!");
    }
    //height width
    int h = infoIn.height;
    int w = infoIn.width;
